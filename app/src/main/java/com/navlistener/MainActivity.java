...
public class MainActivity extends AppCompatActivity {
    ...
    private final BroadcastReceiver navReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (NavListenerService.ACTION_NAV_UPDATE.equals(action)) {
                onNavUpdate(intent.getStringExtra("json"));
    ...
    private void setupMqtt() {
        ...
        mqttManager.setStatusListener(new MqttManager.StatusListener() {
            @Override public void onConnected()    { runOnUiThread(() -> updateMqttStatus()); }
            @Override public void onDisconnected() { runOnUiThread(() -> updateMqttStatus()); }
            ...
            @Override public void onError(String error) {
                runOnUiThread(() -> {
                    updateMqttStatus();
                    Toast.makeText(MainActivity.this, "MQTT Error: " + error, Toast.LENGTH_SHORT).show();
                });
            }
        });
    }
    ...
    private void setupButtons() {
        btnPermission.setOnClickListener(v -> {
            if (!isNotificationListenerEnabled()) showPermissionDialog();
            else Toast.makeText(this, "✓ Izin sudah aktif", Toast.LENGTH_SHORT).show();
        });
    ...
    private void updateListenerStatus() {
        ...
        tvListenerStatus.setTextColor(ContextCompat.getColor(this, ok ? R.color.green : R.color.red));
    ...
    private void updateMqttStatus() {
        ...
        tvMqttStatus.setText(ok ? "Terhubung ke " + MqttManager.BROKER_HOST : "Tidak terhubung");
        tvMqttStatus.setTextColor(ContextCompat.getColor(this, ok ? R.color.green : R.color.red));
    ...
    private void onNavUpdate(String jsonStr) {
        ...
        try {
            ...
            tvDistance.setText(unit.equals("km") ? String.format("%.1f km", dist) : String.format("%.0f m", dist));
            tvStreet.setText(street);
            tvEta.setText("ETA " + eta);
            tvRemain.setText((remUnit.equals("km") ? String.format("%.1f km", remain) : String.format("%.0f m", remain)) + " lagi");
